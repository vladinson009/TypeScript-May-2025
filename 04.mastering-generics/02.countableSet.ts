...
class CountedSet<T> implements CountableSet<T> {
  ...
  private getCount<AS extends T>(item: AS): number | undefined {
    return this.items.get(item);
  }
  add(item: T): void {
    const currentCount = this.getCount(item);
    currentCount ? this.items.set(item, currentCount + 1) : this.items.set(item, 1);
  }
  remove(item: T): void {
    const currentCount = this.getCount(item);
    currentCount && this.items.set(item, currentCount - 1);
  }
  contains(item: T): boolean {
